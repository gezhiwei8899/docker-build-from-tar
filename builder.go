...
func (b *Builder) pullArtifact() error{
	...
	command := []string{
		...
		fmt.Sprintf("--localpath=%s", "app.tar.gz"),
		fmt.Sprintf("--path=%s", filepath.Join(b.ArtifactPath, "app.tar.gz")),
		fmt.Sprintf("--tag=%s", b.ArtifactTag),
	}
	if _, err := (CMD{command, "/root/src"}).Run(); err != nil {
